...
class website_marketplace_dashboard(http.Controller):
    ...
    def submit_as_seller(self, **post):
        ...
        if country_id and url_handler:
            ...
            internal_group = request.env['ir.model.data'].sudo().check_object_reference('base', 'group_user')[1]
            internal_group_obj = request.env["res.groups"].browse(internal_group)
            if internal_group_obj and current_user: 
                current_user.write({'groups_id':[(6,0,[internal_group_obj.id])]})
